...
function Navbar(){
  ...
  return (
    <>
      <nav className = {`h-16 flex sm:flex-row sm:justify-between flex-col items-center transition-all duration-500 ${scrollNavbar ? "fixed top-[-12px] z-[100] backdrop-brightness-[.4] backdrop-blur border-b-solid border-b-4 border-b-white max-sm:left-[-20px] max-sm:top-[-16px] max-sm:h-22" : "border-b-transparent border-b-4  flex sm:flex-row sm:justify-between flex-col items-center"}`}>
        ...
        <ul className = {`flex flex-col items-center mt-3 sm:hidden transition-all duration-300 backdrop-blur origin-top  ${toggleMenu ? "h-[500px] scale-y-100" : "h-[0] scale-y-0"} ${scrollNavbar ? "bg-white h-[200px] z-[100]" : "text-black "}`}>
              <li className = "p-5 transition-colors active:text-white dutration-200 cursor-pointer sm:text-[0.9rem] text-[1.5rem] active:bg-blue-200  w-screen sm:w-auto sm:active:border-transparent text-center"><Link to = "hero" spy = {true} duration = {500} offset = {-49}>home</Link></li>
              <li className = "p-5 transition-all active:text-white dutration-200 cursor-pointer sm:text-[0.9rem] text-[1.5rem] active:bg-blue-200  w-screen sm:w-auto sm:active:border-transparent text-center"><Link to = "content" spy = {true} duration = {500} offset = {-49}>skills</Link></li>
              <li className = "p-5 transition-all active:text-white dutration-200 cursor-pointer sm:text-[0.9rem] text-[1.5rem] active:bg-blue-200  w-screen sm:w-auto sm:active:border-transparent text-center"><Link to = "about-me" spy = {true} duration = {500} offset = {-49}>about</Link></li>
